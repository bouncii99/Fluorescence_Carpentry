...
def outline(image, threshold, iteration, kernel_size, maxlevel):
	...
	# plt.xticks([]), plt.yticks([])
	# plt.show()
	# print(len(cv2.findContours(closing,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)))
	img, contours, hierarchy = cv2.findContours(closing,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE) #find contours with simple approximation
	# print(type(closing))
	# print('closing clean:\n', closing)
	# clean_closing = copy.deepcopy(closing)
	cv2.imshow('clean', closing) #Figure 3
	...
	single_area = cv2.contourArea(contours[1])
	...
	# cleaner_closing = copy.deepcopy(closing)
	# print(type(closing))
	# print(len(closing), len(closing[0]))
	# total = 0
	...
	# # closing_fraction = total/area_total
	# # print(total)
	# # print(area_total)
	# # print(closing_fraction)


	

	# # print(total)
	# # if np.all(clean_closing == cleaner_closing):
	# # 	print('True')
	# # else:
	# # 	print('False')
